...
class Mystrategy(StrategyBase):
    ...
    def on_bar(self, bar):
        ...
        if (barMin+1) % self.K_min==0 and self.K_minCounter>=self.K_min:
            ...
            #self.pltUpdate('up')
            if self.combineBar():self.pltUpdate()#更新combinedBar
    ...
    def update_dailyBarMin(self,bar):
        ...
        self.dailyBarMin.loc[self.dailyBarMin.shape[0]] = \
            [self.dailyBar.ix[rownum - self.K_min]['strdatetime'],
             ...
             max(self.dailyBar.iloc[rownum - self.K_min:rownum - 1]['high']),  # 合并周期内最高价
             min(self.dailyBar.iloc[rownum - self.K_min:rownum - 1]['low']),  # 合并周期内的最低价
...
#            print self.combindBar.loc[0]
    ...
    def trendJudge(self):
        ...
        if self.trendList[-1]==1 and self.trendList[-2]==-1:#出现反弹行情
            ...
            if self.marketCount==1:#第一波行情，起点做高点看
            ...
            self.trendPeriod=0
            barrow = self.combindBar.shape[0]
            if self.marketCount==1:#第一波行情，起点做低点看
    ...
    def pltPrepare(self):
        ...
        self.axesSet['up'] = self._Fig.add_axes([0.1, 0.68, 0.8, 0.3], axis_bgcolor='black')
        self.axesSet['mid'] = self._Fig.add_axes([0.1, 0.34, 0.8, 0.25], axis_bgcolor='black')
        self.axesSet['down'] = self._Fig.add_axes([0.1, 0.08, 0.8, 0.15], axis_bgcolor='black')
        self.plotSet['up'] = pc.PlotCandlesticks(axesObject=self.axesSet['up'], pdata=self.dailyBarMin, xlength=300, showX=True, name="daialy Bar")
        self.plotSet['mid'] = pc.PlotCandlesticks(axesObject=self.axesSet['mid'],pdata=self.combindBar,xlength=300,showX=True,name="combined Bar")
        self.plotSet['down'] = pc.PlotPositionLine(axesObject=self.axesSet['down'],pdata=self.positionTrend,xlength=300,showX=False,name="position trend")
