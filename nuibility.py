...
class SubPlot_BasicInfo:
    ...
    def plot_codesymbol(self, xbase, ybase):
        ...
        label = self._Axes.text(xbase, ybase, txtstr, fontproperties=__font_properties__, verticalalignment='top',
    ...
    def plot_companylocation(self, xbase, ybase):
        ...
        txtstr = self._cominfo[u'公司概况'][u'区域'] + '   ' + self._cominfo[u'公司概况'][u'所属行业'] + '   ' + \
                 self._cominfo[u'发行相关'][u'上市日期']
    ...
    def plot_mainbusiness(self, xbase, ybase):
        ...
        for sizelimit, linelimit, fontsize, yshift in lookups:
            if length <= sizelimit:
                txtstr = '\n'.join(
                    [txtstr[linelimit * idx: linelimit * (idx + 1)] for idx in range(length // linelimit + 1)])
                fsize = fontsize
                ycoord = ybase - yshift
                break

        label = self._Axes.text(xbase, ycoord, txtstr, fontproperties=__font_properties__, verticalalignment='top',
    ...
    def plot_sortinginfo(self, xbase, ybase):
        ...
        for idx in range(len(infolist) // 10 + 1):
            ...
            label = self._Axes.text(xcoord, ybase, txtstr, fontproperties=__font_properties__, verticalalignment='top',
                                    horizontalalignment='left', color='blue')
...
class SubPlot_PriceBase:
    ...
    def __init__(self, pdata, parent, xparams, name):
        ...
        self._zipoc = zip(self._open, self._close)#smart:将open和close一对对打包
        ...
        if u'开盘二' in quotes:
            ...
            self._up_2 = numpy.array(
                [True if po < pc and po is not None else False for po, pc in self._zipoc_2])  # 标示出该天股价日内上涨的一个序列
            self._down_2 = numpy.array(
                [True if po > pc and po is not None else False for po, pc in self._zipoc_2])  # 标示出该天股价日内下跌的一个序列
            self._side_2 = numpy.array(
                [True if po == pc and po is not None else False for po, pc in self._zipoc_2])  # 标示出该天股价日内走平的一个序列
    ...
    def plot_candlestick(self):
        ...
        if True in up:
            axes.vlines(xindex[up], rarray_low[up], rarray_high[up], edgecolor='red', linewidth=0.6, label='_nolegend_',
                        alpha=0.5)
            axes.vlines(xindex[up], rarray_open[up], rarray_close[up], edgecolor='red', linewidth=3.0,
                        label='_nolegend_', alpha=0.5)

        if True in down:
            axes.vlines(xindex[down], rarray_low[down], rarray_high[down], edgecolor='green', linewidth=0.6,
                        label='_nolegend_', alpha=0.5)
            axes.vlines(xindex[down], rarray_open[down], rarray_close[down], edgecolor='green', linewidth=3.0,
                        label='_nolegend_', alpha=0.5)

        if True in side:
            axes.vlines(xindex[side], rarray_low[side], rarray_high[side], edgecolor='0.7', linewidth=0.6,
                        label='_nolegend_', alpha=0.5)
            axes.vlines(xindex[side], rarray_open[side], rarray_close[side], edgecolor='0.7', linewidth=3.0,
                        label='_nolegend_', alpha=0.5)
    ...
    def plot_candlestick_2(self):
        ...
        # XXX: 可以使用 alpha 参数调节透明度
        if True in up:
            axes.vlines(xindex[up], rarray_low[up], rarray_high[up], edgecolor='0.7', linewidth=0.6, label='_nolegend_',
                        alpha=0.5)
            axes.vlines(xindex[up], rarray_open[up], rarray_close[up], edgecolor='0.7', linewidth=3.0,
                        label='_nolegend_', alpha=0.5)

        if True in down:
            axes.vlines(xindex[down], rarray_low[down], rarray_high[down], edgecolor='0.3', linewidth=0.6,
                        label='_nolegend_', alpha=0.5)
            axes.vlines(xindex[down], rarray_open[down], rarray_close[down], edgecolor='0.3', linewidth=3.0,
                        label='_nolegend_', alpha=0.5)

        if True in side:
            axes.vlines(xindex[side], rarray_low[side], rarray_high[side], edgecolor='1.0', linewidth=0.6,
                        label='_nolegend_', alpha=1.0)
            axes.vlines(xindex[side], rarray_open[side], rarray_close[side], edgecolor='1.0', linewidth=3.0,
                        label='_nolegend_', alpha=1.0)
    ...
    def plot_capitalinfo(self):
        ...
        def find_biggestblank(didx):
            ...
            diff, hi, low = max(
                [(math.log(compseq[idx] / float(compseq[idx + 1]), expbase), compseq[idx], compseq[idx + 1]) for idx in
                 range(len(compseq) - 1)])
        ...
        label = axes.text(length - 1, find_biggestblank(didx=length - 1), infostr, fontproperties=__font_properties__,
                          horizontalalignment='right', color='0.7')
        label.set_fontsize(fsize)
        #   label.set_zorder(0)     # XXX: 放在底层
    ...
    def plot_usernotes(self):
        ...
        for note in usernotes:
            if note[u'类型'] == u'筛选结果':
                ...
                axes.arrow(xpos, ypos, dx, dy, facecolor=color, edgecolor=color, linewidth=0.7, head_width=0.6,
    ...
    def plot_vlines(self):
        ...
        # XXX: 可以使用 alpha 参数调节透明度
        if True in up:
            axes.vlines(xindex[up], rarray_low[up], rarray_high[up], edgecolor='red', linewidth=lwidth,
                        label='_nolegend_', alpha=0.5)

        if True in down:
            axes.vlines(xindex[down], rarray_low[down], rarray_high[down], edgecolor='green', linewidth=lwidth,
                        label='_nolegend_', alpha=0.5)

        if True in side:
            axes.vlines(xindex[side], rarray_low[side], rarray_high[side], edgecolor='0.7', linewidth=lwidth,
                        label='_nolegend_', alpha=0.5)
    ...
    def plot_vlines_2(self):
        ...
        # XXX: 可以使用 alpha 参数调节透明度
        if True in up:
            axes.vlines(xindex[up], rarray_low[up], rarray_high[up], edgecolor='0.7', linewidth=lwidth,
                        label='_nolegend_', alpha=0.5)

        if True in down:
            axes.vlines(xindex[down], rarray_low[down], rarray_high[down], edgecolor='0.3', linewidth=lwidth,
                        label='_nolegend_', alpha=0.5)

        if True in side:
            axes.vlines(xindex[side], rarray_low[side], rarray_high[side], edgecolor='1.0', linewidth=lwidth,
                        label='_nolegend_', alpha=1.0)
...
class SubPlot_PriceFullSpan(SubPlot_PriceBase):
    ...
    def plot_windowspan(self):
        ...
        window = matplotlib.patches.Rectangle((xcoord, ycoord), width, height, fill=False,
                                              edgecolor=__color_lightblue__, linewidth=0.3, alpha=0.7)
...
class SubPlot_Price(SubPlot_PriceBase):
    ...
    def plot_pricenotes(self):
        ...
        if u'开盘二' in quotes:
            ...
            for iy, iy2 in zip(sorted(majorticks[:-1] + minorticks[1:-1]),
                               sorted(majorticks_2[:-1] + minorticks_2[1:-1])):
...
class SubPlot_TORateBase:
    ...
    def plot_torate(self):
        ...
        if True in up:
            axes.vlines(xindex[up], tozeros[up], rarray_to[up], edgecolor='red', linewidth=lwidth, label='_nolegend_',
                        alpha=0.5)
        if True in down:
            axes.vlines(xindex[down], tozeros[down], rarray_to[down], edgecolor='green', linewidth=lwidth,
                        label='_nolegend_', alpha=0.5)
        if True in side:
            axes.vlines(xindex[side], tozeros[side], rarray_to[side], edgecolor='0.7', linewidth=lwidth,
                        label='_nolegend_', alpha=0.5)
    ...
    def plot_torate_2(self):
        ...
        # XXX: 如果 up/down/side 各项全部为 False，那么 vlines() 会报错。
        if True in up:
            axes.vlines(xindex[up], tozeros[up], rarray_to[up], edgecolor='0.7', linewidth=lwidth, label='_nolegend_',
                        alpha=alpha)
        if True in down:
            axes.vlines(xindex[down], tozeros[down], rarray_to[down], edgecolor='0.3', linewidth=lwidth,
    ...
    def set_yticks(self):
        ...
        # ==================================================================================================================================================
        if self._name == 'torate':
            minorticks = list(itertools.chain.from_iterable(
                mi for mi in [[ma + (tostep / 4.0) * i for i in range(1, 4)] for ma in majorticks]))
            yMinorLocator = FixedLocator(numpy.array(minorticks))
            yaxis.set_minor_locator(yMinorLocator)
...
class MyFigure:
    ...
    def __init__(self, pdata):
        ...
        # ===============================================================================================================
        self._Fig = pyplot.figure(figsize=(self._xlength, self._ylength), dpi=self._figdpi,
    ...
    def _compute_size(self):
        ...
        x_financial, y_financial = self._subplots['financial'].get_size() if 'financial' in self._subplots else (
        0.0, 0.0)
    ...
    def _compute_rect(self):
        ...
        if 'price' in self._subplots:
            rect = (
            (x_left + (x_all - x_left - x_right - x_price) / 2) / x_all, (y_bottom + y_torate) / y_all, x_price / x_all,
            y_price / y_all)  # K线图部分
            rects['price'] = rect

        if 'torate' in self._subplots:
            rect = ((x_left + (x_all - x_left - x_right - x_torate) / 2) / x_all, y_bottom / y_all, x_torate / x_all,
                    y_torate / y_all)  # 成交量部分
            rects['torate'] = rect

        if 'pricefs' in self._subplots:
            rect = ((x_left + (x_all - x_left - x_right - x_pricefs) / 2) / x_all,
                    (y_all - y_top - y_basic - y_gap1 - y_pricefs) / y_all, x_pricefs / x_all, y_pricefs / y_all)
            rects['pricefs'] = rect

        if 'toratefs' in self._subplots:
            rect = ((x_left + (x_all - x_left - x_right - x_toratefs) / 2) / x_all,
                    (y_bottom + y_torate + y_price + y_gap2) / y_all, x_toratefs / x_all, y_toratefs / y_all)
            rects['toratefs'] = rect
    ...
    def _compute_xparams(self):
        ...
        datelist = [datetime.date(int(ys), int(ms), int(ds)) for ys, ms, ds in
                    [dstr.split('-') for dstr in quotes[u'日期'][sidx:eidx]]]
    ...
    def _compute_xparams_fullspan(self):
        ...
        datelist = [datetime.date(int(ys), int(ms), int(ds)) for ys, ms, ds in
                    [dstr.split('-') for dstr in quotes[u'日期']]]
    ...
    def savefig(self, figpath):
        ...
        保存图片
        '''
        self._Fig.savefig(figpath, dpi=self._figdpi, facecolor=self._figfacecolor, edgecolor=self._figedgecolor,
                          linewidth=self._figlinewidth)
